(function ($) {
	...
	var envisionBlocksGoogleMaps = function ($scope, $) {
		...
		function initialize() {
			...
			var map = new google.maps.Map(
				$scope.find(".envision-blocks-google-map")[0],
				map_options
			);
			...
			for (i = 0; i < locations.length; i++) {
				...
				if ("" != lat.length && "" != lng.length) {
					...
					if (enable_iw && "iw_open" == click_open) {
						var content_markup =
							'<div class="envision-blocks-infowindow-content"><div class="envision-blocks-google-maps-infowindow-title">' +
							title +
							"</div>";

						if ("" != description.length) {
							content_markup +=
								'<div class="envision-blocks-google-maps-infowindow-description">' +
								description +
								"</div>";
						}
						content_markup += "</div>";
	...
	jQuery(window).on("elementor/frontend/init", () => {
		elementorFrontend.hooks.addAction(
			"frontend/element_ready/envision-blocks-google-maps.default",
			envisionBlocksGoogleMaps
		);
	});
})(jQuery);
